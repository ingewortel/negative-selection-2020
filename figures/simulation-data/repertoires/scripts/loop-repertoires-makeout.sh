	...
	for NTRAIN in $SELFSIZES ; do
		...
		for R in $Rvec ; do
			...
			if [ -e $TRAINFILE ] ; then
				...
				echo -e "$TRAINFILE:\n\t@"bash scripts/train-epitopes.sh $TRAINSELF $NTRAIN -T $TYPE -b $pow -r $R "> \$@"
				#sleep 0
			...
			echo "$OUT : $TRAINFILE"
			echo -e "\t@"bash scripts/repertoires.sh \$@ $MODELDIR -s $TRAINFILE -n $MOTIFLENGTH -r $R -m $MATCHTYPE -l $L \&\& \\
			echo -e "\t"bash ../../scripts/analysis/print-progress.sh $REPERTOIREDIR $repnum $OUT
